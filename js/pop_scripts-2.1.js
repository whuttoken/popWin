...
function popWin(fill,custom){
	...
	this.createPop = function(){ //创建弹窗层，并在网页中插入
		if(this._isCreate){alert("pop has exist!");return;}
		var stylePop = "min-height:"+ custom.minHeight +"px;max-height:auto;min-width:"+ custom.minWidth +"px;max-width:"+ custom.maxWidth +"px;background-color:"+ custom.bgColor +";position:absolute;top:100px;display:none;color:"+ custom.fontColor +";font-family:'微软雅黑', '黑体', '宋体';font-size:18px;z-index:1001;";
		...
		var styleTitle = "width:auto;height:40px;text-align:center;margin:10px;line-height:40px;border-bottom:"+ custom.titleLine +";";
	...
	this.setPop = function(){ //初始化弹窗在页面中的位置
	...
	this.fillPop = function(){ //给弹窗添加内容
		$("#"+this._titleId).html($(fill).children().eq(0).html());
		$("#"+this._contentId).html($(fill).children().eq(1).html());
		$(fill).children().detach();
		this._pWidth = $("#"+this._popId).width();
		this._pHeight = $("#"+this._popId).height();
	}
	this.openPop = function(){ //打开弹窗
		this.fillPop();
		this.setPop();
		$("#"+this._popId).fadeIn();
		$("#"+this._maskId).fadeIn();
	};
	this.destroyPop = function(){ //销毁弹窗层
		$("#"+this._popId).detach();
		$("#"+this._maskId).detach();
		$(fill).html(fillHtml);
		this._isCreate = false;
	};
	this.closePop = function(){ //关闭弹窗
		$("#"+this._popId).fadeOut();
		var Obj = this;
	...
	function exist(id){ //判断id在页面中是否存在，若已经存在则对id添加后缀
...
$.fn.popWin = function(custom){ //给jQuery对象添加popWin()方法
		...
		$(this).click(function(){ 
			...
			{ //根据用户参数对默认配置进行重写
				...
				if(custom.titleLine !== undefined)dft.titleLine = custom.titleLine;
			...
			var _popWin = new popWin(this,dft); //创建一个弹窗对象
			_popWin.initPop(); //调用该弹窗对象的初始化方法
			initPopJs(_popWin);
		}); 
} 
